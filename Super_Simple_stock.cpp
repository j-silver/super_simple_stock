...
double Stock::pe_ratio() const
{
	...
	if (common) {
		...
		else pe = static_cast<double>(price)/fix_dividend;
	}
...
double geometric_mean(const std::vector<double>& prices)
{
	...
	double geom_mean {std::accumulate(prices.cbegin(),
					  				  prices.cend(),
					  				  1.0,
...
{
	unsigned total_quantity {std::accumulate(trades.cbegin(),
										trades.cend(),
										unsigned {0},
	...
	double numerator {std::accumulate(trades.cbegin(),
									  trades.cend(),
									  0.0,
...
std::ostream& operator<<(std::ostream& os, const std::vector<Stock>& stocks)
{
	...
	for (const auto& s : stocks) {
		std::string Type {s.is_common() == true ? "common" : "prefer"};
